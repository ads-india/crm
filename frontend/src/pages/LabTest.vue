<template>
  ...
  <div v-if="labTest.doc" ref="parentRef" class="flex h-full">
    <Resizer
      v-if="labTest.doc"
          ...
          <div class="flex gap-4 items-center">
            <div class="group relative h-15.5 w-15.5">
              <Avatar
                size="3xl"
                class="h-15.5 w-15.5"
                :label="labTest.doc.customer_name || labTest.doc.name"
                :image="labTest.doc.image"
              />
            </div>
            <div class="flex flex-col gap-2 truncate text-ink-gray-9">
              <div class="truncate text-2xl font-medium">
                <span>{{ labTest.doc.customer_name || labTest.doc.name }}</span>
              </div>
              <div
                v-if="labTest.doc.mobile_no"
                class="flex items-center gap-1.5 text-base text-ink-gray-8"
              >
                <PhoneIcon class="h-4 w-4" />
                <span>{{ labTest.doc.mobile_no }}</span>
              </div>
              <div
                v-if="labTest.doc.email"
                class="flex items-center gap-1.5 text-base text-ink-gray-8"
              >
                <EmailIcon class="h-4 w-4" />
                <span>{{ labTest.doc.email }}</span>
              </div>
            </div>
          </div>
          <div class="flex gap-1.5">
            <Button
              v-if="callEnabled && labTest.doc.mobile_no"
              :label="__('Make Call')"
              size="sm"
              @click="callEnabled && makeCall(labTest.doc.mobile_no)"
            >
              <template #prefix>
                <PhoneIcon class="h-4 w-4" />
              </template>
...
const { document: labTest } = useDocument('Yorecare Enquiry', props.labTestId)

const breadcrumbs = computed(() => {
  let items = [{ label: __('Lab Tests'), route: { name: 'Lab Tests' } }]
  ...
  items.push({
    label: title.value,
    route: { name: 'Lab Test', params: { labTestId: props.labTestId } },
  })
  return items
})
...
const title = computed(() => {
  ...
  return labTest.doc?.[t] || props.labTestId
})
...
const leads = createResource({
  ...
  cache: ['leads', props.labTestId],
  params: {
    crm_lead: props.labTestId,
  },
  auto: true,
})
