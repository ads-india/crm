<template>
  ...
  <div v-if="enquiry.doc" ref="parentRef" class="flex h-full">
    <Resizer
      v-if="enquiry.doc"
          ...
          <div class="flex gap-4 items-center">
            <div class="group relative h-15.5 w-15.5">
              <Avatar
                size="3xl"
                class="h-15.5 w-15.5"
                :label="enquiry.doc.customer_name || enquiry.doc.name"
                :image="enquiry.doc.image"
              />
            </div>
            <div class="flex flex-col gap-2 truncate text-ink-gray-9">
              <div class="truncate text-2xl font-medium">
                <span>{{ enquiry.doc.customer_name || enquiry.doc.name }}</span>
              </div>
              <div
                v-if="enquiry.doc.mobile_no"
                class="flex items-center gap-1.5 text-base text-ink-gray-8"
              >
                <PhoneIcon class="h-4 w-4" />
                <span>{{ enquiry.doc.mobile_no }}</span>
              </div>
              <div
                v-if="enquiry.doc.email"
                class="flex items-center gap-1.5 text-base text-ink-gray-8"
              >
                <EmailIcon class="h-4 w-4" />
                <span>{{ enquiry.doc.email }}</span>
              </div>
            </div>
          </div>
          <div class="flex gap-1.5">
            <Button
              v-if="callEnabled && enquiry.doc.mobile_no"
              :label="__('Make Call')"
              size="sm"
              @click="callEnabled && makeCall(enquiry.doc.mobile_no)"
            >
              <template #prefix>
                <PhoneIcon class="h-4 w-4" />
              </template>
...
const breadcrumbs = computed(() => {
  ...
  items.push({
    label: title.value,
    route: { name: 'Yorecare Enquiry', params: { yorecareEnquiryId: props.yorecareEnquiryId } },
  })
  return items
})
...
const title = computed(() => {
  ...
  return enquiry.doc?.[t] || props.yorecareEnquiryId
})
...
const leads = createResource({
  ...
  cache: ['leads', props.yorecareEnquiryId],
  params: {
    crm_lead: props.yorecareEnquiryId,
  },
  auto: true,
})
