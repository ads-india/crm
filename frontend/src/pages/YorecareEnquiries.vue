<template>
  <LayoutHeader>
    <template #left-header>
      <ViewBreadcrumbs v-model="viewControls" routeName="Yorecare Enquiries" />
    </template>
    <template #right-header>
      <CustomActions
        v-if="enquiryListView?.customListActions"
  ...
  <KanbanView
    ...
    :options="{
      getRoute: (row) => ({
        name: row.type === 'Medicine Order' ? 'Medicine Order' : 'Yorecare Enquiry',
        params: row.type === 'Medicine Order' 
          ? { medicineOrderId: row.name } 
          : { yorecareEnquiryId: row.name },
  ...
  <div v-else-if="enquiries.data" class="flex h-full items-center justify-center">
    <div class="flex flex-col items-center gap-3 text-xl font-medium text-ink-gray-4">
      ...
      <span>{{ __('No {0} Found', [__('Enquiries')]) }}</span>
      <Button :label="__('Create')" @click="showEnquiryModal = true">
        <template #prefix><FeatherIcon name="plus" class="h-4" /></template>
      </Button>
    </div>
  </div>
  <YorecareEnquiryModal
...
import YorecareEnquiriesListView from '@/components/ListViews/YorecareEnquiriesListView.vue'
import KanbanView from '@/components/Kanban/KanbanView.vue'
import YorecareEnquiryModal from '@/components/Modals/YorecareEnquiryModal.vue'
...
const enquiryListView = ref(null)
...
const rows = computed(() => {
  ...
  } else if (enquiries.value.data.view_type === 'kanban') {
    return getKanbanRows(enquiries.value.data.data, enquiries.value.data.fields)
  } else {
...
function getGroupedByRows(listRows, groupByField, columns) {
  ...
  groupByField.options?.forEach((option) => {
    ...
    if (groupByField.fieldname == 'medicine_order_status') {
      groupDetail.icon = () =>
        h(IndicatorIcon, {
          class: getEnquiryStatusColor(option),
        })
    }
...
function parseRows(rows, columns = []) {
  let view_type = enquiries.value.data.view_type
    ...
    enquiries.value?.data.rows.forEach((row) => {
      ...
      if (row == 'crm_lead') {
        ...
        _rows[row] = {
          ...
          color: getEnquiryStatusColor(enquiry.medicine_order_status),
        }
      } else if (row == 'type') {
        _rows[row] = {
          label: enquiry.type,
...
function getEnquiryStatusColor(status) {
  const statusColors = {
    'New': 'blue',
    'In Progress': 'orange',
    'Completed': 'green',
...
function onNewClick(column) {
  let column_field = enquiries.value.params.column_field
